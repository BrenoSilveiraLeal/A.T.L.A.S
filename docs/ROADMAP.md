...
**Modo Paper Trading (25/09/2026):** fluxo transacional separado com proposta guiada por análise SMA3/SMA8, aprovação manual, limites/caixa reservados, ordem virtual, fill condicionado a uma nova cotação atrasada, posição, livro balanceado e auditoria. Implementado no Supabase Free e verificado em PGlite; a fonte brapi passou em smoke de leitura. A validação do fluxo completo no navegador com a sessão AAL2 do proprietário permanece pendente. Preenchimento é acionado manualmente; não há prova de concorrência remota. [Manual e limitações](PAPER_TRADING.md). Isso não muda os estados PENDING de execução real, nuvem contínua, corretora, Pix ou supervisor.
...
| 8 · Treasury | Ledger balanceado, alocações, reservas, depósitos, retiradas e confirmação independente. | RPCs e concorrência verificadas em Postgres; soma dos saldos reconciliada; QR/instrução não credita caixa; funding automatizado apenas se oficial. | PENDING — B02, B05, B07. Ledger **virtual** de Paper Trading implementado separadamente; não supre conciliação real. |
