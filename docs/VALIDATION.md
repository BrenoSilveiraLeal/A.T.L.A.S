...
## Atualização de 25/09/2026 — Paper Trading

O projeto Supabase ATLAS `bxikkprpvfirjlmnxqhh` estava `INACTIVE` no começo da continuação; a restauração gratuita foi solicitada e o estado passou a `ACTIVE_HEALTHY`. A migration `20260925040950_atlas_paper_trading.sql` foi aplicada remotamente como versão `20260925040950`. As sete tabelas `paper_*` têm RLS ativo, `anon` não possui `SELECT` e `service_role` não possui `INSERT` direto; as mutações passam por RPCs restritas. O projeto tinha zero contas/ordens paper e zero ordens reais após a aplicação; `live_trading_enabled=false`. O Security Advisor apresentou apenas o aviso existente de proteção contra senhas vazadas desativada, recurso [documentado pelo Supabase](https://supabase.com/docs/guides/auth/password-security#password-strength-and-leaked-password-protection). O Performance Advisor apresentou avisos informativos de índices de chaves estrangeiras, sem bloqueio funcional; [orientação oficial](https://supabase.com/docs/guides/database/database-linter?lint=0001_unindexed_foreign_keys).

No PostgreSQL WASM/PGlite, a suíte nova comprovou proposta idempotente → aprovação com reserva → cotação anterior sem fill → cotação posterior com BUY/SELL virtual → caixa/posição/P&L → lançamentos balanceados, além de recusa por orçamento, venda sem posição, isolamento anônimo e pausa/cancelamento. Isso é **validação local transacional**, não prova de concorrência remota ou de execução financeira. `tests/paper.test.ts` verificou dados stale, identidade e SMA. A simulação real no navegador ainda exige login AAL2 e um ativo compatível com o token brapi da instalação; não foi criada conta virtual nem ordem remota em nome do proprietário durante a verificação.

O código de análise, o serviço e a tela estão em `src/core/paper.ts`, `src/lib/paper.ts` e `src/components/paper-panel.tsx`. Nenhum código externo foi copiado. Fonte/limites do modo em [PAPER_TRADING.md](PAPER_TRADING.md). Resultados históricos do restante deste arquivo não comprovam a nova tela.

Verificações finais desta continuação: `npm run lint` passou; `npm run typecheck` passou; `npm test` passou com **448 testes, 4 opt-in ignorados, 25 arquivos**; `npm run test:mt5` passou com **40 testes**; `npm run build` passou no OneDrive nesta execução, sem `EPERM`. O smoke `ATLAS_PAPER_PUBLIC_SMOKE=1` passou com cotação/histórico públicos atuais; o smoke de dados públicos da análise existente também passou. `node scripts/verify-supabase.mjs` passou com **38 tabelas sensíveis negadas ao anônimo**, incluindo as sete `paper_*`. `npm run local` iniciou a versão compilada e o scheduler registrou `OK` (0 jobs, pois não havia agentes habilitados). `/login` retornou 200; `/app/paper` redirecionou ao login (307) e `/api/atlas/paper` recusou acesso anônimo (401). A tela autenticada e a criação de conta/ordem virtual no Supabase real permanecem sem smoke do usuário AAL2; nenhum dado financeiro ou virtual remoto foi criado apenas para testes.
