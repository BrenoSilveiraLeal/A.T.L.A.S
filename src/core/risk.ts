...
function validLimits(limits: RiskLimits): boolean {
  const positive = [
    limits.maxPositionPerAgent,
    limits.maxPortfolioExposure,
    limits.maxOrderValue,
    limits.maxDailyLoss,
    limits.maxWeeklyLoss,
    limits.maxDrawdownBps,
    limits.maxSectorExposure,
    limits.maxCorrelatedExposure,
    limits.newsEmergencyThreshold,
  ];
  return (
    positive.every(isPositiveAmount) &&
    ...
    [
      ...
      limits.maxQuoteAgeMs,
      limits.maxBrokerSnapshotAgeMs,
    ].every((value) => isShareQuantity(value)) &&
...
function validSnapshot(snapshot: RiskSnapshot): boolean {
  return (
    [
      ...
      snapshot.agentCashAvailable,
      snapshot.equity,
      snapshot.portfolioExposure,
      snapshot.agentExposure,
      snapshot.sectorExposure,
      snapshot.correlatedExposure,
      snapshot.dailyLoss,
      snapshot.weeklyLoss,
    ...
    [
      ...
      snapshot.reservedSellQuantity,
      snapshot.openPositions,
      snapshot.ordersLastMinute,
      snapshot.ordersToday,
    ].every((value) => isShareQuantity(value, true)) &&
    ...
    decimal(snapshot.correlatedExposure).lte(
      decimal(snapshot.portfolioExposure),
    )
  );
}
...
export function evaluateTrade(
  ...
  deny(
    context.corporateActionsVerified !== true,
    "CORPORATE_ACTION_REVIEW_REQUIRED",
  );
  deny(context.session !== "OPEN", "MARKET_NOT_OPEN");

  try {
    if (!context.limits || !validLimits(context.limits)) {
      reasons.add("RISK_CONFIGURATION_REQUIRED");
      return finish();
    }
    ...
    checkAge(
      proposal.dataTimestamp,
      limits.maxQuoteAgeMs,
      "STALE_PROPOSAL_DATA",
    );
    deny(
      timestamp(proposal.expiresAt) <= now ||
        timestamp(proposal.expiresAt) <= timestamp(proposal.createdAt),
      "PROPOSAL_EXPIRED",
    );
    deny(
      !proposal.id ||
    ...
    deny(
      !context.authorizedSymbols.includes(proposal.symbol),
    ...
    deny(
      !context.allowedOrderKinds.includes(proposal.kind),
      "UNSUPPORTED_ORDER_KIND",
    );
    // A standalone STOP has no guaranteed execution-price bound in this initial cash-equity kernel.
    deny(
      proposal.kind !== "LIMIT" && proposal.kind !== "STOP_LIMIT",
      "UNBOUNDED_ORDER",
    );
    deny(!isPositiveAmount(proposal.limitPrice), "INVALID_LIMIT_PRICE");
    if (!context.instrument) {
      reasons.add("INSTRUMENT_METADATA_REQUIRED");
      return finish();
    }
    const instrument = context.instrument;
    deny(
      instrument.symbol !== proposal.symbol ||
        instrument.exchange !== "B3" ||
        instrument.assetClass !== "EQUITY",
      "UNSUPPORTED_INSTRUMENT",
    );
    deny(instrument.tradingEnabled !== true, "INSTRUMENT_NOT_TRADABLE");
    if (
      !isShareQuantity(instrument.lotSize) ||
      !isPositiveAmount(instrument.priceTick)
    ) {
      reasons.add("INVALID_INSTRUMENT_METADATA");
      return finish();
    }
    deny(
      isShareQuantity(proposal.quantity) &&
        proposal.quantity % instrument.lotSize !== 0,
      "INVALID_LOT_SIZE",
    );
    if (isPositiveAmount(proposal.limitPrice))
      deny(
        !decimal(proposal.limitPrice)
          .mod(decimal(instrument.priceTick))
          .isZero(),
        "INVALID_PRICE_TICK",
      );
    if (proposal.kind === "STOP_LIMIT") {
      ...
      if (
        isPositiveAmount(proposal.limitPrice) &&
        isPositiveAmount(proposal.stopPrice)
      ) {
        deny(
          !decimal(proposal.stopPrice)
            .mod(decimal(instrument.priceTick))
            .isZero(),
          "INVALID_STOP_TICK",
        );
        deny(
          proposal.side === "BUY" &&
            decimal(proposal.limitPrice).lt(decimal(proposal.stopPrice)),
          "INVALID_STOP_LIMIT_RELATION",
        );
        deny(
          proposal.side === "SELL" &&
            decimal(proposal.limitPrice).gt(decimal(proposal.stopPrice)),
          "INVALID_STOP_LIMIT_RELATION",
        );
      }
    ...
    if (!isPositiveAmount(quote.bid) || !isPositiveAmount(quote.ask)) {
      reasons.add("INVALID_QUOTE");
      return finish();
    }
    deny(decimal(quote.bid).gt(decimal(quote.ask)), "CROSSED_QUOTE");

    if (!context.snapshot || !validSnapshot(context.snapshot)) {
      reasons.add("INVALID_BROKER_SNAPSHOT");
      return finish();
    }
    ...
    deny(
      !context.accountId ||
        ...
        snapshot.agentId !== proposal.agentId ||
        snapshot.symbol !== proposal.symbol,
    ...
    deny(
      decimal(snapshot.portfolioExposure).gt(decimal(snapshot.equity)),
      "LEVERAGE_DETECTED",
    );
    deny(
      decimal(snapshot.dailyLoss).gte(decimal(limits.maxDailyLoss)),
      "DAILY_LOSS_LIMIT",
    );
    deny(
      decimal(snapshot.weeklyLoss).gte(decimal(limits.maxWeeklyLoss)),
      "WEEKLY_LOSS_LIMIT",
    );
    deny(
      decimal(snapshot.drawdownBps).gte(decimal(limits.maxDrawdownBps)),
      "DRAWDOWN_LIMIT",
    );
    deny(
      snapshot.ordersLastMinute >= limits.maxOrdersPerMinute,
      "MINUTE_ORDER_LIMIT",
    );
    deny(snapshot.ordersToday >= limits.maxOrdersPerDay, "DAILY_ORDER_LIMIT");
    if (
      !isNonNegativeAmount(context.estimatedFees) ||
      !isNonNegativeAmount(context.newsEmergencyScore) ||
      decimal(context.newsEmergencyScore).gt(1)
    ) {
      reasons.add("INVALID_COST_OR_NEWS_STATE");
      return finish();
    }
    deny(
      decimal(context.newsEmergencyScore).gte(
        decimal(limits.newsEmergencyThreshold),
      ),
      "NEWS_EMERGENCY",
    );
    if (
    ...
    estimatedDebit = money(
      proposal.side === "BUY" ? debit : decimal(context.estimatedFees),
    );
    ...
    const adverse =
      proposal.side === "BUY"
        ? decimal(proposal.limitPrice).minus(reference)
        : reference.minus(decimal(proposal.limitPrice));
    deny(
      adverse.div(reference).mul(10_000).gt(decimal(limits.maxSlippageBps)),
      "SLIPPAGE_LIMIT",
    );
    ...
    if (proposal.side === "BUY") {
      ...
      deny(
        debit.gt(decimal(snapshot.agentCashAvailable)),
        "INSUFFICIENT_AGENT_CASH",
      );
      deny(
        decimal(snapshot.cashAvailable)
          .minus(debit)
          .lt(decimal(limits.minCashReserve)),
        "CASH_RESERVE_LIMIT",
      );
      deny(
        decimal(snapshot.agentExposure)
          .plus(value)
          .gt(decimal(limits.maxPositionPerAgent)),
        "AGENT_EXPOSURE_LIMIT",
      );
      deny(
        decimal(snapshot.portfolioExposure)
          .plus(value)
          .gt(decimal(limits.maxPortfolioExposure)),
        "PORTFOLIO_EXPOSURE_LIMIT",
      );
      deny(
        decimal(snapshot.portfolioExposure)
          .plus(debit)
          .gt(decimal(snapshot.equity)),
        "LEVERAGE_FORBIDDEN",
      );
      deny(
        decimal(snapshot.sectorExposure)
          .plus(value)
          .gt(decimal(limits.maxSectorExposure)),
        "SECTOR_EXPOSURE_LIMIT",
      );
      deny(
        decimal(snapshot.correlatedExposure)
          .plus(value)
          .gt(decimal(limits.maxCorrelatedExposure)),
        "CORRELATED_EXPOSURE_LIMIT",
      );
      deny(
        snapshot.ownedQuantity === 0 &&
          snapshot.openPositions >= limits.maxOpenPositions,
        "OPEN_POSITION_LIMIT",
      );
    } else {
      deny(
        proposal.quantity >
          snapshot.ownedQuantity - snapshot.reservedSellQuantity,
