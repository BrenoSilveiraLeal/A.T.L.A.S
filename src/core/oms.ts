...
export function createOrder(
  ...
  [
    ...
    accountId,
    clientOrderId,
  ].forEach(assertIdentity);
  timestamp(proposal.createdAt);
  if (
    !isShareQuantity(proposal.quantity) ||
...
export function transitionOrder(
  ...
  if (timestamp(at) < timestamp(order.updatedAt))
    throw new Error("NON_MONOTONIC_ORDER_TIME");
  if (!TRANSITIONS[order.state]?.includes(next))
    throw new Error(`INVALID_ORDER_TRANSITION: ${order.state} -> ${next}`);
  if (next === "FILLED" && order.filledQuantity !== order.proposal.quantity)
    throw new Error("FILL_EVIDENCE_REQUIRED");
  if (
    next === "PARTIALLY_FILLED" &&
    !(
      order.filledQuantity > 0 && order.filledQuantity < order.proposal.quantity
    )
  )
    throw new Error("FILL_EVIDENCE_REQUIRED");
  if (
    (next === "SUBMITTED" || next === "CANCEL_REQUESTED") &&
    !order.brokerOrderId
  )
    throw new Error("BROKER_ACK_REQUIRED");
...
export function acknowledgeSubmission(
  ...
  if (order.brokerOrderId && order.brokerOrderId !== brokerOrderId)
  ...
  return transitionOrder(
    { ...order, brokerOrderId, lastError: null },
    "SUBMITTED",
    at,
  );
}
...
export function markSubmissionUnknown(
  ...
  return {
    ...transitionOrder(order, "SUBMISSION_UNKNOWN", at),
    lastError: reason,
  };
}
...
function sameExecution(left: BrokerExecution, right: BrokerExecution): boolean {
  return (
    ...
    decimal(left.price).eq(decimal(right.price)) &&
    decimal(left.fees).eq(decimal(right.fees)) &&
...
export function applyExecution(
  ...
  order: ManagedOrder;
  account: LedgerAccount;
  entry: ExecutionLedgerEntry | null;
  duplicate: boolean;
  breaches: readonly string[];
} {
  [fill.executionId, fill.brokerOrderId, fill.accountId].forEach(
    assertIdentity,
  );
  if (
    !isShareQuantity(fill.quantity) ||
    !isPositiveAmount(fill.price) ||
    !isNonNegativeAmount(fill.fees)
  )
    throw new Error("INVALID_EXECUTION");
  const executed = timestamp(fill.executedAt);
  const received = timestamp(receivedAt);
  if (executed > received || executed < timestamp(order.proposal.createdAt))
    throw new Error("INVALID_EXECUTION_TIME");
  if (received < timestamp(order.updatedAt))
    throw new Error("NON_MONOTONIC_ORDER_TIME");
  if (
    fill.accountId !== order.accountId ||
    fill.accountId !== account.accountId ||
    fill.brokerOrderId !== order.brokerOrderId ||
    fill.symbol !== order.proposal.symbol ||
    fill.symbol !== account.symbol ||
    fill.side !== order.proposal.side ||
    account.agentId !== order.proposal.agentId
  )
    throw new Error("EXECUTION_IDENTITY_MISMATCH");
  const existing = order.executions.find(
    (execution) => execution.executionId === fill.executionId,
  );
  if (existing) {
    if (!sameExecution(existing, fill))
      throw new Error("EXECUTION_ID_CONFLICT");
    return { order, account, entry: null, duplicate: true, breaches: [] };
  }
  if (
    ["CREATED", "RISK_REVIEW", "APPROVED", "REJECTED", "FILLED"].includes(
      order.state,
    )
  )
    throw new Error("EXECUTION_STATE_MISMATCH");
  const filledQuantity = order.filledQuantity + fill.quantity;
  if (
    !isShareQuantity(filledQuantity) ||
    filledQuantity > order.proposal.quantity
  )
    throw new Error("OVERFILL_RECONCILIATION_REQUIRED");
  if (
    !isShareQuantity(account.quantity, true) ||
    !isNonNegativeAmount(account.costBasis) ||
    !isNonNegativeAmount(account.totalFees) ||
    (account.quantity === 0 && !decimal(account.costBasis).isZero())
  )
  ...
  if (fill.side === "BUY") {
    ...
    if (!isShareQuantity(quantity))
      throw new Error("INVALID_POSITION_QUANTITY");
    cash = cash.minus(value).minus(fees);
    basis = basis.plus(value).plus(fees);
  } else {
    if (fill.quantity > quantity)
      throw new Error("SHORT_FILL_RECONCILIATION_REQUIRED");
    const removedBasis =
      fill.quantity === quantity
        ? basis
        : decimal(money(basis.mul(fill.quantity).div(quantity)));
    quantity -= fill.quantity;
    cash = cash.plus(value).minus(fees);
    basis = basis.minus(removedBasis);
    pnlDelta = value.minus(fees).minus(removedBasis);
  }
  const nextAccount: LedgerAccount = {
    ...account,
    quantity,
    cash: money(cash),
    costBasis: money(basis),
    realizedPnl: money(decimal(account.realizedPnl).plus(pnlDelta)),
    totalFees: money(decimal(account.totalFees).plus(fees)),
  };
  // Cancelled/expired orders may have delayed fills executed before the terminal event.
  const state: OrderState =
    filledQuantity === order.proposal.quantity
      ? "FILLED"
      : ["CANCEL_REQUESTED", "CANCELLED", "EXPIRED"].includes(order.state)
        ? order.state
        : "PARTIALLY_FILLED";
  const nextOrder: ManagedOrder = {
    ...
    state,
    filledQuantity,
    filledNotional: money(decimal(order.filledNotional).plus(value)),
    fees: money(decimal(order.fees).plus(fees)),
    executions: [...order.executions, { ...fill }],
    updatedAt: receivedAt,
    version: order.version + 1,
  };
  const requested =
    order.proposal.limitPrice === null
      ? decimal(fill.price)
      : decimal(order.proposal.limitPrice);
  const priceDifference =
    fill.side === "BUY"
      ? decimal(fill.price).minus(requested)
      : requested.minus(decimal(fill.price));
  const breaches: string[] = [];
  if (cash.lt(0)) breaches.push("CASH_DEFICIT_AFTER_EXECUTION");
  if (order.proposal.limitPrice !== null && priceDifference.gt(0))
    breaches.push("LIMIT_PRICE_VIOLATION");
  const entry: ExecutionLedgerEntry = {
    ...
    side: fill.side,
    quantity: fill.quantity,
    requestedPrice: order.proposal.limitPrice,
    executedPrice: money(fill.price),
    fees: money(fees),
    slippage: money(priceDifference.mul(fill.quantity)),
    cashBefore: money(cashBefore),
    cashAfter: nextAccount.cash,
    quantityBefore: account.quantity,
    quantityAfter: quantity,
    costBasisBefore: account.costBasis,
    costBasisAfter: nextAccount.costBasis,
    realizedPnlDelta: money(pnlDelta),
    source: "BROKER_EXECUTION",
    reason: order.proposal.reasoningSummary,
    strategyVersion: order.proposal.strategyVersion,
...
export function reconcileAccount(
  ...
  try {
    ...
    if (!local.accountId || local.accountId !== broker.accountId)
      discrepancies.push("ACCOUNT_MISMATCH");
    if (local.complete !== true || broker.complete !== true)
      discrepancies.push("INCOMPLETE_SNAPSHOT");
    for (const [name, snapshot] of [
      ...
      if (age < 0 || age > options.maxAgeMs)
        discrepancies.push(`${name}_SNAPSHOT_TIME_INVALID`);
    }
    if (
      decimal(local.cash)
        .minus(decimal(broker.cash))
        .abs()
        .gt(decimal(tolerance))
    )
      discrepancies.push("CASH_MISMATCH");
    const positions = (
      ...
      for (const position of snapshot.positions) {
        if (
          !/^[A-Z0-9]{4,16}$/.test(position.symbol) ||
          !isShareQuantity(position.quantity, true) ||
          result.has(position.symbol)
        )
          throw new Error("INVALID_POSITION_SNAPSHOT");
        result.set(position.symbol, position.quantity);
      }
      return result;
    };
    const localPositions = positions(local),
      brokerPositions = positions(broker);
    for (const symbol of [
      ...new Set([...localPositions.keys(), ...brokerPositions.keys()]),
    ].sort()) {
      if (
        (localPositions.get(symbol) ?? 0) !== (brokerPositions.get(symbol) ?? 0)
      )
        discrepancies.push(`POSITION_MISMATCH:${symbol}`);
    }
  } catch {
    discrepancies.push("INVALID_RECONCILIATION_INPUT");
  }
  return {
