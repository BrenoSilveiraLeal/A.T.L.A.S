...
export function decimal(value: DecimalString): Decimal {
  if (typeof value !== "string" || !AMOUNT.test(value)) {
    throw new Error(
      "INVALID_DECIMAL: expected a plain decimal string, at most 18 integer and 8 fractional digits",
    );
  }
...
export function sum(values: readonly DecimalString[]): DecimalString {
  return money(
    values.reduce((total, value) => total.plus(decimal(value)), decimal("0")),
  );
}

export function isNonNegativeAmount(value: unknown): value is DecimalString {
  try {
    return typeof value === "string" && decimal(value).gte(0);
  } catch {
    return false;
  }
}
...
export function isPositiveAmount(value: unknown): value is DecimalString {
  try {
    return typeof value === "string" && decimal(value).gt(0);
  } catch {
    return false;
  }
}

export function isShareQuantity(
  value: unknown,
  allowZero = false,
): value is number {
  return (
