...
function decimal(value: string, positive = false): Decimal {
  if (
    ...
    throw new ResearchError("INVALID_DECIMAL");
  const result = new D(value);
  if (!result.isFinite() || (positive && !result.gt(0)))
    throw new ResearchError("INVALID_DECIMAL");
  return result;
}
function time(value: string): number {
  if (
    ...
    throw new ResearchError("INVALID_TIMESTAMP");
...
function periodCheck(period: number) {
  if (!Number.isSafeInteger(period) || period < 1)
    throw new ResearchError("INVALID_PERIOD");
}
...
export function ema(
  ...
  return prices.map((price, index) => {
    ...
    previous =
      previous === null
        ? seed.div(period)
        : price.mul(alpha).plus(previous.mul(new D(1).minus(alpha)));
...
export function rsi(values: readonly string[], period = 14): (string | null)[] {
  ...
  return prices.map((price, index) => {
    ...
    const gain = D.max(delta, 0);
    const loss = D.max(delta.negated(), 0);
    if (index <= period) {
      gains = gains.plus(gain);
      losses = losses.plus(loss);
    }
    if (index < period) return null;
    if (index === period) {
      gains = gains.div(period);
      losses = losses.div(period);
    } else {
      gains = gains
        .mul(period - 1)
        .plus(gain)
    ...
    return new D(100)
      .minus(new D(100).div(new D(1).plus(gains.div(losses))))
      .toFixed();
  });
}
...
export function atr(
  bars: readonly Pick<HistoricalBar, "high" | "low" | "close">[],
  period = 14,
): (string | null)[] {
  periodCheck(period);
  let previousClose: Decimal | null = null;
  let average: Decimal | null = null;
  let seed = new D(0);
  return bars.map((bar, index) => {
    const high = decimal(bar.high, true);
    const low = decimal(bar.low, true);
    const close = decimal(bar.close, true);
    if (high.lt(low) || close.gt(high) || close.lt(low))
      throw new ResearchError("INVALID_OHLC");
    const range =
      previousClose === null
        ? high.minus(low)
        : D.max(
...
export type ResearchStrategy = (
  context: Readonly<SignalContext>,
) => ResearchSignal;
...
function validateBars(bars: readonly ResearchBar[], asOf: number) {
  ...
  bars.forEach((bar, index) => {
    const opened = time(bar.timestamp);
    const closed = time(bar.closedAt);
    const available = time(bar.availableAt);
    if (opened >= closed || available < closed)
      throw new ResearchError("INVALID_BAR_TIMING");
    if (closed > asOf || available > asOf)
      throw new ResearchError("FUTURE_DATA");
    if (index && time(bars[index - 1].closedAt) > opened)
      throw new ResearchError("OVERLAPPING_BARS");
    const open = decimal(bar.open, true);
    const high = decimal(bar.high, true);
    const low = decimal(bar.low, true);
    const close = decimal(bar.close, true);
    if (
    ...
    if (!Number.isSafeInteger(bar.volume) || bar.volume < 0)
      throw new ResearchError("INVALID_VOLUME");
  });
}
...
function prepare(dataset: ResearchDataset) {
  ...
  if (dataset.priceBasis !== "RAW")
    throw new ResearchError("RAW_PRICES_REQUIRED");
  validateBars(dataset.bars, asOf);
  const lastClose = time(dataset.bars[dataset.bars.length - 1].closedAt);
  if (
    !Array.isArray(dataset.corporateActions) ||
    time(dataset.corporateActionsVerifiedThrough) < lastClose
  )
    throw new ResearchError("CORPORATE_ACTIONS_UNVERIFIED");
  if (time(dataset.corporateActionsVerifiedThrough) > asOf)
    throw new ResearchError("FUTURE_ACTION_COVERAGE");
  const opens = new Set(dataset.bars.map((bar) => time(bar.timestamp)));
  const actionIds = new Set<string>();
  const actions = dataset.corporateActions.map((action) => {
    const effective = time(action.effectiveAt);
    const available = time(action.availableAt);
    if (!action.id || actionIds.has(action.id))
      throw new ResearchError("DUPLICATE_ACTION");
    actionIds.add(action.id);
    if (available > effective || available > asOf)
      throw new ResearchError("ACTION_NOT_AVAILABLE");
    if (!opens.has(effective))
      throw new ResearchError("ACTION_OUTSIDE_OPEN_BOUNDARY");
    if (action.type === "SPLIT") decimal(action.ratio, true);
    else if (action.type === "CASH_DIVIDEND") {
      decimal(action.cashPerShare, true);
      if (time(action.payableAt) < effective)
        throw new ResearchError("INVALID_DIVIDEND_PAYMENT_DATE");
    } else throw new ResearchError("UNSUPPORTED_CORPORATE_ACTION");
    return Object.freeze({ ...action });
  });
  const actionTimes = actions.map((action) => action.effectiveAt);
  if (new Set(actionTimes).size !== actionTimes.length)
    throw new ResearchError("SIMULTANEOUS_ACTIONS_REQUIRE_ORDERING");
  const observations = (dataset.observations ?? []).map((observation) => {
    if (
      time(observation.timestamp) > time(observation.availableAt) ||
      time(observation.availableAt) > asOf
    )
      throw new ResearchError("INVALID_OBSERVATION_AVAILABILITY");
    decimal(observation.value);
    return Object.freeze({ ...observation });
  });
  const bars = dataset.bars.map((bar) =>
    Object.freeze({ ...bar, adjustedClose: null }),
  );
...
export function runBacktest(
  ...
  if (
    !Number.isSafeInteger(config.periodsPerYear) ||
    config.periodsPerYear <= 0 ||
    !Number.isFinite(config.annualRiskFreeRate) ||
    config.annualRiskFreeRate <= -1
  )
    throw new ResearchError("INVALID_ANNUALIZATION");
  const cost = Object.fromEntries(
    Object.entries(config.costs).map(([key, value]) => [key, decimal(value)]),
  );
  for (const key of [
    "brokerageFixed",
    "brokerageBps",
  ...
  let income = new D(0);
  let entryAt = "";
  let pending: { signal: ResearchSignal; decisionAt: string } | null = null;
  ...
  let totalNotional = new D(0);
  let exposedBars = 0;
  const settle = (at: number) => {
    receivables.forEach((receivable) => {
      if (!receivable.paid && receivable.payableAt <= at) {
        cash = cash.plus(receivable.value);
  ...
  const outstanding = () =>
    receivables.reduce(
      (sum, receivable) => (receivable.paid ? sum : sum.plus(receivable.value)),
      new D(0),
    );
  ...
  for (let index = 0; index < bars.length; index++) {
    ...
    const closed = time(bar.closedAt);
    actions
      .filter((action) => time(action.effectiveAt) === opened)
      .forEach((action) => {
        if (action.type === "SPLIT") {
          const shares = quantity.mul(action.ratio);
          if (!shares.isInteger())
            throw new ResearchError("FRACTIONAL_SPLIT_UNSUPPORTED");
          quantity = shares;
          ...
          if (pending?.signal.action === "BUY") {
            rejected.push({
              ...
              attemptedAt: bar.timestamp,
              reason: "CORPORATE_ACTION_INVALIDATED_SIGNAL",
            });
            pending = null;
          }
          ...
          receivables.push({
            value: entitlement,
            payableAt: time(action.payableAt),
            paid: false,
          });
    ...
    if (pending && pending.signal.action !== "HOLD") {
      ...
      if (time(decisionAt) > opened)
        throw new ResearchError("EXECUTION_BEFORE_DECISION");
      if (signal.action === "BUY" && quantity.gt(0))
        reason = "POSITION_ALREADY_OPEN";
      if (signal.action === "SELL" && quantity.isZero())
        reason = "NO_LONG_POSITION";
      const shares =
        signal.action === "BUY" ? decimal(signal.quantity, true) : quantity;
      if (!shares.isInteger())
        throw new ResearchError("INTEGER_QUANTITY_REQUIRED");
      const price = new D(bar.open).mul(
        signal.action === "BUY"
          ? new D(1).plus(friction)
          : new D(1).minus(friction),
      );
      const notional = shares.mul(price);
      const fees = cost.brokerageFixed.plus(notional.mul(feeRate));
      if (signal.action === "BUY" && notional.plus(fees).gt(cash))
        reason = "INSUFFICIENT_CASH";
      if (signal.action === "SELL" && notional.lte(fees))
        reason = "FEES_EXCEED_PROCEEDS";
      if (reason)
        rejected.push({ decisionAt, attemptedAt: bar.timestamp, reason });
      else {
        if (signal.action === "BUY") {
          cash = cash.minus(notional).minus(fees);
          quantity = shares;
          basis = notional.plus(fees);
          income = new D(0);
          ...
          trades.push({
            ...
            pnl: proceeds.minus(basis).plus(income).toFixed(),
    ...
    equity.push({
      timestamp: bar.closedAt,
      cash: cash.toFixed(),
      quantity: quantity.toFixed(),
      receivables: outstanding().toFixed(),
      equity: cash.plus(quantity.mul(bar.close)).plus(outstanding()).toFixed(),
    });
    const context: SignalContext = {
      ...
      bars: Object.freeze(
        bars
          .slice(0, index + 1)
          .filter((item) => time(item.availableAt) <= closed),
      ),
      observations: Object.freeze(
        observations.filter((item) => time(item.availableAt) <= closed),
      ),
      corporateActions: Object.freeze(
        actions.filter((item) => time(item.availableAt) <= closed),
      ),
      cash: cash.toFixed(),
      quantity: quantity.toFixed(),
    };
    const signal = strategy(Object.freeze(context));
    if (!signal || !["BUY", "SELL", "HOLD"].includes(signal.action))
      throw new ResearchError("INVALID_SIGNAL");
    if (signal.action === "BUY" && !decimal(signal.quantity, true).isInteger())
      throw new ResearchError("INTEGER_QUANTITY_REQUIRED");
  ...
  return {
    ...
    metrics: metrics(
      ...
      totalNotional,
      exposedBars,
      time(bars[0].timestamp),
      config,
    ),
    finalCash: cash.toFixed(),
    finalQuantity: quantity.toFixed(),
    finalReceivables: outstanding().toFixed(),
    unexecutedFinalSignal:
      pending && pending.signal.action !== "HOLD" ? pending.signal : null,
  };
}
...
function metrics(
  ...
  const returns = values.map((value, index) =>
    value
      .div(index ? values[index - 1] : initial)
      .minus(1)
      .toNumber(),
  );
  const riskFreePerPeriod =
    Math.pow(1 + config.annualRiskFreeRate, 1 / config.periodsPerYear) - 1;
  const excess = returns.map((value) => value - riskFreePerPeriod);
  const mean = excess.reduce((sum, value) => sum + value, 0) / excess.length;
  const variance =
    excess.length > 1
      ? excess.reduce((sum, value) => sum + (value - mean) ** 2, 0) /
        (excess.length - 1)
      : 0;
  ...
  let peak = initial;
  let drawdown = new D(0);
  values.forEach((value) => {
    peak = D.max(peak, value);
    drawdown = D.max(drawdown, peak.minus(value).div(peak));
  });
  const pnls = trades.map((trade) => new D(trade.pnl));
  const wins = pnls.filter((pnl) => pnl.gt(0));
  const losses = pnls.filter((pnl) => pnl.lt(0));
  const sum = (items: Decimal[]) =>
    items.reduce((total, value) => total.plus(value), new D(0));
  const grossProfit = sum(wins);
  const grossLoss = sum(losses).abs();
  const years =
    (time(points[points.length - 1].timestamp) - started) / (365.25 * DAY);
  const averageEquity = sum(values).div(values.length);
  return {
    totalReturn: final.div(initial).minus(1).toNumber(),
    cagr:
      years > 0 && final.gt(0)
        ? finite(Math.pow(final.div(initial).toNumber(), 1 / years) - 1)
        : null,
    sharpe:
      variance > 0 && excess.length > 1
        ? finite(
            (mean / Math.sqrt(variance)) * Math.sqrt(config.periodsPerYear),
          )
        : null,
    sortino:
      downside > 0 && excess.length > 1
        ? finite((mean / downside) * Math.sqrt(config.periodsPerYear))
        : null,
    maxDrawdown: drawdown.toNumber(),
    profitFactor: grossLoss.gt(0)
    ...
    averageLoss: losses.length
      ? sum(losses).div(losses.length).toFixed()
      : null,
    exposure: exposed / points.length,
    turnover: averageEquity.gt(0)
      ? notional.div(averageEquity).toNumber()
      : null,
    totalFees: fees.toFixed(),
  };
}

export interface TemporalPartitions {
  train: readonly ResearchBar[];
  validation: readonly ResearchBar[];
  outOfSample: readonly ResearchBar[];
  trainCutoff: string;
...
function frozenPartition(bars: readonly ResearchBar[]): readonly ResearchBar[] {
  ...
  if (bars.some((bar) => time(bar.availableAt) > cutoff))
    throw new ResearchError("PARTITION_DATA_NOT_AVAILABLE");
  return Object.freeze(
    bars.map((bar) => Object.freeze({ ...bar, adjustedClose: null })),
  );
}
...
export function temporalSplit(
  ...
  periodCheck(trainSize);
  periodCheck(validationSize);
  if (trainSize + validationSize >= bars.length)
    throw new ResearchError("EMPTY_OUT_OF_SAMPLE");
  const train = frozenPartition(bars.slice(0, trainSize));
  const validation = frozenPartition(
    bars.slice(trainSize, trainSize + validationSize),
  );
  ...
  return {
    ...
    validation,
    outOfSample,
    trainCutoff: train[train.length - 1].closedAt,
    validationCutoff: validation[validation.length - 1].closedAt,
  };
}
...
export function walkForward(
  ...
  validateBars(bars, Date.now());
  periodCheck(config.trainSize);
  periodCheck(config.validationSize);
  periodCheck(config.testSize);
  const step = config.stepSize ?? config.testSize;
  const purge = config.purgeBars ?? 0;
  if (
    !Number.isSafeInteger(step) ||
    step < config.testSize ||
    !Number.isSafeInteger(purge) ||
    purge < 0
  )
    throw new ResearchError("OVERLAPPING_TEST_WINDOWS");
  const windowSize =
    config.trainSize + config.validationSize + config.testSize + 2 * purge;
  ...
  for (let start = 0; start + windowSize <= bars.length; start += step) {
    ...
    const validation = frozenPartition(
      bars.slice(validationStart, validationStart + config.validationSize),
    );
    const outOfSample = frozenPartition(
      bars.slice(testStart, testStart + config.testSize),
    );
    folds.push({
      train,
      validation,
