  ...
  items: z
    .array(
      z.object({
        ...
        titulo: z.string().min(1).max(2000),
        introducao: z.string().max(20_000),
        data_publicacao: z.string(),
        link: z.string().url().max(4000),
        editorias: z.string().max(1000),
      }),
    )
    .max(20),
});
...
export async function fetchNews(): Promise<NewsArticle[]> {
  ...
  const articles = payload.items.map((item): NewsArticle => {
    ...
    return {
      ...
      url: canonicalNewsUrl(item.link),
      title,
      summary,
      publishedAt: checkedTimestamp(
        "ibge",
        parseBrazilDate("ibge", item.data_publicacao, true),
        now,
      ),
      retrievedAt,
      contentHash,
      eventClusterId: `exact:${contentHash}`,
      companies: [],
      tickers: [],
      category: item.editorias.split(",").includes("economicas")
        ? "MACRO"
        : "INSTITUTIONAL",
      relevance: null,
      sentiment: null,
  ...
  return deduplicateNews(articles).sort((a, b) =>
