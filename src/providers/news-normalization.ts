...
export function newsHash(title: string, summary: string): string {
  const normalized = `${title} ${summary}`.normalize("NFKC").toLocaleLowerCase("pt-BR").replace(/\s+/g, " ").trim();
  return createHash("sha256").update(normalized).digest("hex");
}

export function canonicalNewsUrl(value: string): string {
  let url: URL;
  try { url = new URL(value); } catch { throw new ProviderError("ibge", "INVALID_RESPONSE"); }
...
export function deduplicateNews(articles: NewsArticle[]): NewsArticle[] {
  ...
  return articles.filter((article) => {
    ...
    urls.add(article.url); hashes.add(article.contentHash);
    return true;
  });
}
