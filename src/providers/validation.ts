...
export const tickerSchema = z.string().trim().toUpperCase().regex(/^[A-Z]{4}\d{1,2}F?$/);
export const decimalSchema = z.union([
  z.number().finite(),
  z.string().max(40).regex(/^-?\d+(?:\.\d+)?$/),
]).transform((value) => new Decimal(value).toFixed());
export const positivePriceSchema = decimalSchema.refine((value) => new Decimal(value).gt(0));
...
export function checkedTimestamp(provider: ProviderName, value: string | number, now: number): string {
  const timestamp = typeof value === "number" ? value : Date.parse(value);
  if (!Number.isFinite(timestamp) || timestamp < 0) throw new ProviderError(provider, "INVALID_RESPONSE");
...
function partsAt(timestamp: number) {
  return Object.fromEntries(brazilParts.formatToParts(timestamp).map(({ type, value }) => [type, value]));
}
...
export function parseBrazilDate(provider: ProviderName, value: string, withTime = false): string {
