    ...
    .array(
      z.object({
        ...
        data: z.object({
          ...
          regularMarketPrice: positivePriceSchema,
...
function token(): string | undefined {
  return process.env.BRAPI_API_TOKEN?.trim() || undefined;
}

function validateIdentity(
  row:
    | z.infer<typeof quoteSchema>["results"][number]
    | z.infer<typeof historySchema>["results"][number],
  ticker: string,
) {
  if (row.changed || row.symbol !== ticker)
    throw new ProviderError("brapi", "TICKER_CHANGED");
  if (row.requestedSymbol !== ticker)
    throw new ProviderError("brapi", "INVALID_RESPONSE");
}
...
export async function fetchQuote(input: string): Promise<MarketQuote> {
  ...
  const payload = parseResponse(
    "brapi",
    quoteSchema,
    await getJson("brapi", url, { token: token() }),
  );
  const now = Date.now();
  checkedTimestamp("brapi", payload.requestedAt, now);
  const row = payload.results[0];
  validateIdentity(row, ticker);
  const dataTimestamp = checkedTimestamp(
    "brapi",
    row.data.regularMarketTime,
    now,
  );
  if (Date.parse(dataTimestamp) > Date.parse(payload.requestedAt))
    throw new ProviderError("brapi", "INVALID_RESPONSE");
  const ageSeconds = Math.floor((now - Date.parse(dataTimestamp)) / 1000);
  return {
    ticker,
    price: row.data.regularMarketPrice,
    currency: "BRL",
    dataTimestamp,
    retrievedAt: new Date(now).toISOString(),
    source: url.toString(),
    feed: "DELAYED",
    freshness: ageSeconds > 45 * 60 ? "STALE" : "DELAYED",
    ageSeconds,
    tradable: false,
  };
}
...
export async function fetchHistory(input: string): Promise<MarketHistory> {
  ...
  const row = payload.results[0];
  validateIdentity(row, ticker);
  if (!row.data.historicalDataPrice.length)
    throw new ProviderError("brapi", "EMPTY_DATA");
  const seen = new Set<number>();
  const bars = row.data.historicalDataPrice
    .map((bar) => {
...
export async function searchAssets(
  ...
  return payload.results
    ...
    .map((row) => ({
      ticker: checkedTicker(row.symbol),
      name: row.name,
      exchange: "B3",
      currency: "BRL",
      assetType: "stock",
