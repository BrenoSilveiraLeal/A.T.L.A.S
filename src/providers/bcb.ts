...
const macroSchema = z
  .array(z.object({ data: z.string(), valor: decimalSchema }))
  .max(40);

function sgsDate(date: string): string {
...
export async function fetchMacro(): Promise<MacroObservation[]> {
  ...
  url.search = new URLSearchParams({
    formato: "json",
    dataInicial: sgsDate(startDate),
    dataFinal: sgsDate(endDate),
  }).toString();
  ...
  return payload
    .map((row): MacroObservation => {
      const dataTimestamp = checkedTimestamp(
        "bcb",
        parseBrazilDate("bcb", row.data),
        Date.parse(retrievedAt),
      );
      const observationDate = brazilDate(Date.parse(dataTimestamp));
      if (observationDate > endDate)
        throw new ProviderError("bcb", "FUTURE_DATA");
      if (observationDate < startDate || seen.has(observationDate))
        throw new ProviderError("bcb", "INVALID_RESPONSE");
