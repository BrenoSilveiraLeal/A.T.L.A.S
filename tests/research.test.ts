...
function bars(prices: string[]): ResearchBar[] {
  return prices.map((price, index) => {
    ...
    return {
      ...
      availableAt: `${date}T20:00:00.000Z`,
      open: price,
      high: "200",
      low: "0.01",
      close: price,
...
function dataset(candles: ResearchBar[]): ResearchDataset {
  return {
    source: "https://example.com/synthetic-test-only",
    asOf: "2025-06-01T00:00:00.000Z",
    priceBasis: "RAW",
    bars: candles,
    corporateActions: [],
    corporateActionsVerifiedThrough: candles[candles.length - 1].closedAt,
  };
}
...
describe("deterministic indicators", () => {
  it("computes SMA and EMA with explicit warmup and SMA seed", () => {
    expect(sma(["1", "2", "3", "4", "8"], 3)).toEqual([
      null,
      null,
      "2",
      "3",
    ...
    expect(ema(["1", "2", "3", "4", "8"], 3)).toEqual([
      null,
      null,
      "2",
      "3",
  ...
  it("implements Wilder RSI with deterministic flat, rising and falling cases", () => {
    ...
    expect(rsi(["10", "11", "10", "11", "10"], 3).slice(3).map(Number)).toEqual(
      [(100 * 2) / 3, (100 * 4) / 9],
    );
  });
  ...
  it("rejects invalid periods, input values and OHLC rather than creating zeroes", () => {
    ...
    expect(() => atr([{ high: "9", low: "10", close: "10" }], 1)).toThrow(
      "INVALID_OHLC",
    );
  });
});
...
describe("causal long-only backtest", () => {
  it("fills only at next opening, never signal close or unseen opening", () => {
    ...
    expect(result.fills[0]).toMatchObject({
      decisionAt: candles[0].closedAt,
      executedAt: candles[1].timestamp,
      price: "11",
      quantity: "2",
    });
    expect(result.fills[1]).toMatchObject({
      executedAt: candles[2].timestamp,
      price: "13",
    });
  ...
  it("only reveals observations and late candles after availableAt", () => {
    ...
    data.observations = [
      {
        ...
        availableAt: candles[2].closedAt,
      },
    ];
    ...
    runBacktest(
      data,
      (context) => {
        seen.push([context.bars.length, context.observations.length]);
        return { action: "HOLD" };
      },
      config,
    );
  ...
  it("accounts for brokerage, exchange fees, full spread and slippage with decimals", () => {
    const result = runBacktest(
      dataset(bars(["10", "10", "12"])),
      (context) =>
        context.bars.length === 1
          ? { action: "BUY", quantity: "2" }
          : context.bars.length === 2
            ? { action: "SELL" }
            : { action: "HOLD" },
      {
        ...config,
  ...
  it("does not generate binary floating-point money drift", () => {
    const result = runBacktest(
      dataset(bars(["0.1", "0.1", "0.2"])),
      (context) =>
        context.bars.length === 1
          ? { action: "BUY", quantity: "3" }
          : { action: "SELL" },
      { ...config, initialCapital: "0.3" },
    );
  ...
  it("forbids short selling, fractional shares and increasing an existing position", () => {
    ...
    ).toThrow("INTEGER_QUANTITY_REQUIRED");
    const repeat = runBacktest(
      dataset(bars(["10", "10", "10"])),
  ...
  it("rejects unknown action, invalid costs and unsupported price basis", () => {
    ...
    expect(() =>
      runBacktest(data, () => ({ action: "SHORT" }) as never, config),
    ).toThrow("INVALID_SIGNAL");
    expect(() =>
      runBacktest(data, () => ({ action: "HOLD" }), {
        ...config,
        costs: { ...config.costs, brokerageFixed: "-1" },
      }),
    ).toThrow("INVALID_COSTS");
    expect(() =>
      runBacktest(
        { ...data, priceBasis: "ADJUSTED" } as never,
        () => ({ action: "HOLD" }),
        config,
      ),
    ).toThrow("RAW_PRICES_REQUIRED");
  });
});
...
describe("corporate actions and point-in-time validation", () => {
  it("blocks missing or incomplete corporate-action coverage", () => {
    ...
    ).toThrow("CORPORATE_ACTIONS_UNVERIFIED");
    expect(() =>
      runBacktest(
        { ...data, corporateActionsVerifiedThrough: data.bars[0].closedAt },
        () => ({ action: "HOLD" }),
        config,
      ),
    ).toThrow("CORPORATE_ACTIONS_UNVERIFIED");
    ...
    data.corporateActions = [
      {
        ...
        type: "SPLIT",
        ratio: "2",
        availableAt: candles[0].closedAt,
        effectiveAt: candles[2].timestamp,
      },
    ];
    const result = runBacktest(
      data,
      (context) =>
        context.bars.length === 1
          ? { action: "BUY", quantity: "1" }
          : context.bars.length === 3
            ? { action: "SELL" }
            : { action: "HOLD" },
      { ...config, initialCapital: "200" },
    );
  ...
  it("blocks fractional reverse split settlement instead of rounding shares or inventing cash", () => {
    ...
    ).toThrow("FRACTIONAL_SPLIT_UNSUPPORTED");
  });
  ...
  it("invalidates an unfilled pre-split buy proposal rather than guessing its revised quantity", () => {
    ...
    data.corporateActions = [
      {
        ...
        ratio: "2",
        availableAt: candles[0].closedAt,
        effectiveAt: candles[1].timestamp,
      },
    ];
    const result = runBacktest(
      data,
    ...
    expect(result.rejected[0].reason).toBe(
      "CORPORATE_ACTION_INVALIDATED_SIGNAL",
    );
  });
  ...
  it("accrues dividend receivable at ex-date and cash only on payable date, even after selling", () => {
    ...
    expect(result.equity[2]).toMatchObject({
      cash: "100",
      receivables: "5",
      equity: "200",
    });
    expect(result.equity[3]).toMatchObject({
      cash: "196",
      quantity: "0",
      receivables: "5",
      equity: "201",
    ...
    data.corporateActions = [
      {
        ...
        type: "SPLIT",
        ratio: "2",
        availableAt: candles[1].closedAt,
        effectiveAt: candles[1].timestamp,
      },
    ];
    expect(() => runBacktest(data, () => ({ action: "HOLD" }), config)).toThrow(
  ...
  it("rejects future as-of, future availability, fabricated dates and overlapping bars", () => {
    ...
    ).toThrow("FUTURE_AS_OF");
    const late = bars(["10", "10"]);
    late[0].availableAt = "2025-07-01T00:00:00Z";
    expect(() =>
      runBacktest(dataset(late), () => ({ action: "HOLD" }), config),
    ).toThrow("FUTURE_DATA");
    const invalid = bars(["10"]);
    invalid[0].timestamp = "2025-02-31T12:00:00Z";
    expect(() =>
      runBacktest(dataset(invalid), () => ({ action: "HOLD" }), config),
    ).toThrow("INVALID_TIMESTAMP");
    const overlap = bars(["10", "10"]);
    overlap[1].timestamp = overlap[0].timestamp;
    expect(() =>
      runBacktest(dataset(overlap), () => ({ action: "HOLD" }), config),
    ).toThrow("OVERLAPPING_BARS");
  });
});
...
describe("research statistics and temporal validation", () => {
  ...
  it("computes closed-trade P&L statistics and close-to-close drawdown including initial equity", () => {
    ...
    const deviation = Math.sqrt(
      excess.reduce((sum, value) => sum + (value - mean) ** 2, 0) / 2,
    );
    expect(result.metrics.sharpe).toBeCloseTo(
      (mean / deviation) * Math.sqrt(252),
      10,
    );
    expect(result.metrics.sortino).toBeCloseTo(
      (mean / Math.sqrt((2 / 101) ** 2 / 3)) * Math.sqrt(252),
      10,
    );
  });
  ...
  it("profit factor and expectancy use completed net P&L, with a losing and a winning trade", () => {
    const result = runBacktest(
      dataset(bars(["10", "10", "12", "10", "9"])),
      (context) =>
        context.bars.length === 1 || context.bars.length === 3
          ? { action: "BUY", quantity: "1" }
          : { action: "SELL" },
      config,
    );
  ...
  it("blocks a training revision published only during validation", () => {
    ...
    expect(() => temporalSplit(candles, 5, 2)).toThrow(
      "PARTITION_DATA_NOT_AVAILABLE",
    );
  });
  ...
  it("walk-forward purges label horizons and never repeats test observations across folds", () => {
    ...
    const folds = walkForward(candles, {
      trainSize: 5,
      validationSize: 2,
      testSize: 3,
      purgeBars: 1,
    ...
    const tests = folds.flatMap((fold) =>
      fold.outOfSample.map((bar) => bar.timestamp),
    );
    expect(new Set(tests).size).toBe(tests.length);
    for (const fold of folds) {
      expect(Date.parse(fold.trainCutoff)).toBeLessThan(
        Date.parse(fold.validation[0].timestamp),
      );
      expect(Date.parse(fold.validationCutoff)).toBeLessThan(
        Date.parse(fold.outOfSample[0].timestamp),
      );
    }
  });
  ...
  it("rejects invalid splits and overlapping walk-forward test steps", () => {
    ...
    ).toThrow("OVERLAPPING_TEST_WINDOWS");
    expect(() =>
      walkForward(candles, { trainSize: 10, validationSize: 2, testSize: 3 }),
    ).toThrow("INSUFFICIENT_WALK_FORWARD_DATA");
  });
});
