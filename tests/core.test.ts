...
function proposal(patch: Partial<TradeProposal> = {}): TradeProposal {
  return {
    ...
    stopPrice: null,
    createdAt: BEFORE,
    expiresAt: "2026-09-11T14:00:30Z",
    dataTimestamp: BEFORE,
    strategyVersion: "strategy-v1",
    reasoningSummary: "Synthetic test fixture",
    ...patch,
  };
}
...
function context(patch: Partial<RiskContext> = {}): RiskContext {
  return {
    ...
    session: "OPEN",
    sessionObservedAt: BEFORE,
    allowedOrderKinds: ["LIMIT", "STOP_LIMIT"],
    authorizedSymbols: ["TEST3"],
    estimatedFees: "1",
    newsEmergencyScore: "0.1",
    limits: {
      maxPositionPerAgent: "500",
      ...
      minCashReserve: "100",
      maxOpenPositions: 5,
      maxSectorExposure: "500",
      maxCorrelatedExposure: "600",
      maxOrdersPerMinute: 3,
      maxOrdersPerDay: 10,
      maxSlippageBps: "100",
      newsEmergencyThreshold: "0.8",
      maxQuoteAgeMs: 60_000,
      maxBrokerSnapshotAgeMs: 60_000,
    ...
    snapshot: {
      ...
      observedAt: BEFORE,
      cashAvailable: "1000",
      agentCashAvailable: "500",
      equity: "2000",
      portfolioExposure: "500",
      agentExposure: "100",
      sectorExposure: "100",
      correlatedExposure: "100",
      ownedQuantity: 0,
...
describe("decimal accounting boundaries", () => {
  it("sums without binary float drift and preserves large decimal precision", () => {
    ...
    expect(
      money(decimal("999999999999999999.99999999").minus("0.00000001")),
    ).toBe("999999999999999999.99999998");
  });
  it.each([
    "NaN",
    "Infinity",
...
describe("deterministic fail-closed risk", () => {
  it("approves a fully configured bounded cash-equity proposal", () => {
    expect(evaluateTrade(proposal(), context())).toEqual({
      approved: true,
      reasons: [],
      notional: "200",
      estimatedDebit: "201",
  ...
  it.each(["DELAYED", "EOD", "STALE", "UNKNOWN"] as const)(
    "does not trade on %s data",
    (quality) => {
      expect(
        evaluateTrade(
          proposal(),
          context({ quote: { ...context().quote!, quality } }),
        ).reasons,
      ).toContain("FEED_NOT_EXECUTABLE");
    },
  );
  it("blocks stale quote, stale broker and stale market-session observations", () => {
    ...
    const result = evaluateTrade(
      proposal(),
      context({
        quote: { ...context().quote!, observedAt: stale },
        snapshot: { ...context().snapshot!, observedAt: stale },
        sessionObservedAt: stale,
      }),
    );
  ...
  it("blocks future observations and stale proposal evidence", () => {
    expect(
      evaluateTrade(
        proposal(),
        context({
          quote: { ...context().quote!, observedAt: "2026-09-11T14:00:02Z" },
        }),
      ).reasons,
    ).toContain("FUTURE_TIMESTAMP");
    expect(
      evaluateTrade(
        proposal({ dataTimestamp: "2026-09-10T14:00:00Z" }),
        context(),
      ).reasons,
    ).toContain("STALE_PROPOSAL_DATA");
  });
  it.each([
    0,
    -1,
    ...
    expect(evaluateTrade(proposal({ quantity }), context()).reasons).toContain(
      "INVALID_QUANTITY",
    );
  });
  it("rejects malformed, unbounded and unsupported instruments/orders", () => {
    expect(
      evaluateTrade(proposal({ symbol: "WINFUT" }), context()).reasons,
    ).toContain("SYMBOL_NOT_AUTHORIZED");
    expect(
      evaluateTrade(
        proposal({ kind: "STOP", limitPrice: null, stopPrice: "100" }),
        context(),
      ).reasons,
    ).toContain("UNBOUNDED_ORDER");
    expect(
      evaluateTrade(
        proposal({ kind: "STOP_LIMIT", stopPrice: "101" }),
        context(),
      ).reasons,
    ).toContain("INVALID_STOP_LIMIT_RELATION");
    expect(
      evaluateTrade(proposal({ limitPrice: "NaN" }), context()).approved,
    ).toBe(false);
    expect(
      evaluateTrade(proposal({ expiresAt: NOW }), context()).reasons,
  ...
  it("requires matching identity, equity class, lot and decimal tick size", () => {
    expect(
      evaluateTrade(
        proposal(),
        context({ snapshot: { ...context().snapshot!, agentId: "agent-2" } }),
      ).reasons,
    ...
    expect(
      evaluateTrade(
        proposal(),
        context({
          instrument: { ...context().instrument!, assetClass: "OPTION" },
        }),
      ).reasons,
    ).toContain("UNSUPPORTED_INSTRUMENT");
    expect(
      evaluateTrade(
        proposal(),
        context({ instrument: { ...context().instrument!, lotSize: 100 } }),
      ).reasons,
    ).toContain("INVALID_LOT_SIZE");
    expect(
      evaluateTrade(proposal({ limitPrice: "100.001" }), context()).reasons,
    ).toContain("INVALID_PRICE_TICK");
  });
  it("uses trusted fee estimates in cash and reserve checks", () => {
    const snapshot = {
      ...context().snapshot!,
      cashAvailable: "200.99",
      agentCashAvailable: "200.99",
    };
    expect(evaluateTrade(proposal(), context({ snapshot })).reasons).toEqual(
      expect.arrayContaining([
        "INSUFFICIENT_CASH",
        "INSUFFICIENT_AGENT_CASH",
        "CASH_RESERVE_LIMIT",
      ]),
    );
  });
  ...
  it("reserves other open sells before allowing a sale", () => {
    ...
    expect(
      evaluateTrade(
        proposal({ side: "SELL", limitPrice: "99" }),
        context({ snapshot }),
      ).reasons,
    ).toContain("SHORT_SELLING_FORBIDDEN");
    expect(
      evaluateTrade(
        proposal({ side: "SELL", quantity: 1, limitPrice: "99" }),
        context({ snapshot }),
      ).approved,
    ).toBe(true);
  });
  it("checks all exposure ceilings, leverage and open positions", () => {
    const limits = {
      ...context().limits!,
      maxPositionPerAgent: "299",
      maxPortfolioExposure: "699",
      maxSectorExposure: "299",
      maxCorrelatedExposure: "299",
      maxOpenPositions: 1,
    };
    const result = evaluateTrade(
      proposal(),
      context({ limits, snapshot: { ...context().snapshot!, equity: "700" } }),
    );
    expect(result.reasons).toEqual(
      expect.arrayContaining([
        "AGENT_EXPOSURE_LIMIT",
  ...
  it("fails closed on non-finite configuration and crossed quotes", () => {
    expect(
      evaluateTrade(
        proposal(),
        context({ limits: { ...context().limits!, maxOrderValue: "NaN" } }),
      ).approved,
    ).toBe(false);
    expect(
      evaluateTrade(
        proposal(),
        context({ quote: { ...context().quote!, bid: "101" } }),
      ).reasons,
    ).toContain("CROSSED_QUOTE");
  });
});
...
describe("OMS lifecycle and execution accounting", () => {
  it("requires risk-review sequencing and broker acknowledgement", () => {
    ...
    expect(() => transitionOrder(order, "SUBMITTING", NOW)).toThrow(
      "INVALID_ORDER_TRANSITION",
    );
    expect(() => transitionOrder(order, "APPROVED", NOW)).toThrow(
      "INVALID_ORDER_TRANSITION",
    );
    expect(() => transitionOrder(submitted(), "FILLED", NOW)).toThrow(
      "FILL_EVIDENCE_REQUIRED",
    );
  });
  it("quarantines a timed-out submission and never permits automatic resend", () => {
    const acknowledged = submitted();
    const sending: ManagedOrder = {
      ...acknowledged,
      brokerOrderId: null,
      state: "SUBMITTING",
    };
    ...
    expect(acknowledgeSubmission(unknown, "broker-1", AFTER).state).toBe(
      "SUBMITTED",
    );
  });
  ...
  it("is idempotent for acknowledgement but rejects conflicting broker IDs", () => {
    ...
    expect(() => acknowledgeSubmission(order, "broker-2", NOW)).toThrow(
  ...
  it("accounts partial fills, fees and final fill exactly once", () => {
    ...
    const duplicate = applyExecution(
      first.order,
      first.account,
      execution(),
      AFTER,
    ...
    const final = applyExecution(
      first.order,
      first.account,
      execution({ executionId: "fill-2", price: "100", executedAt: AFTER }),
      AFTER,
    );
    ...
    expect(final.account).toMatchObject({
      cash: "800",
      quantity: 2,
      costBasis: "200",
      totalFees: "1",
  ...
  it("rejects changed duplicate fills, overfills and cross-account executions", () => {
    ...
    ).toThrow("EXECUTION_ID_CONFLICT");
    expect(() =>
      applyExecution(
        first.order,
        first.account,
        execution({ executionId: "fill-2", quantity: 2 }),
        AFTER,
      ),
    ).toThrow("OVERFILL_RECONCILIATION_REQUIRED");
    expect(() =>
      applyExecution(
        submitted(),
        account(),
        execution({ accountId: "account-2" }),
        NOW,
      ),
  ...
  it("handles fills racing with cancellation, including delayed final evidence", () => {
    ...
    const final = applyExecution(
      cancelled,
      first.account,
      execution({ executionId: "fill-2", executedAt: NOW }),
      AFTER,
    );
  ...
  it("does not fabricate a fill when an order is cancelled", () => {
    const cancelled = transitionOrder(
      transitionOrder(submitted(), "CANCEL_REQUESTED", NOW),
      "CANCELLED",
      NOW,
    );
  ...
  it("computes sell proceeds, remaining basis and realized P&L with fees", () => {
    ...
    const sell = submitted(
      { id: "sell-1", side: "SELL", limitPrice: "105" },
      "broker-sell",
    );
    const fill = execution({
      brokerOrderId: "broker-sell",
      side: "SELL",
      price: "105",
    });
    ...
    const second = applyExecution(
      first.order,
      first.account,
      { ...fill, executionId: "sell-fill-2" },
      AFTER,
    );
  ...
  it("requires reconciliation rather than inventing short inventory", () => {
    ...
    expect(() =>
      applyExecution(sell, account(), execution({ side: "SELL" }), NOW),
    ).toThrow("SHORT_FILL_RECONCILIATION_REQUIRED");
  });
});

describe("account reconciliation", () => {
  const snapshot = (
    patch: Partial<ReconciliationSnapshot> = {},
  ): ReconciliationSnapshot => ({
    accountId: "account-1",
    observedAt: NOW,
    cash: "1000",
    positions: [{ symbol: "TEST3", quantity: 2 }],
    complete: true,
    ...patch,
  });
  ...
  it("matches an exact complete observation", () => {
    expect(
      reconcileAccount(snapshot(), snapshot({ cash: "1000.00" }), options)
        .matches,
    ).toBe(true);
  });
  it("reports cash and positions without changing the ledger", () => {
    ...
    const result = reconcileAccount(
      ...
      snapshot({
        cash: "999",
        positions: [
          { symbol: "TEST3", quantity: 3 },
          { symbol: "OTHR4", quantity: 1 },
        ],
      }),
      options,
    );
    expect(result.discrepancies).toEqual([
      "CASH_MISMATCH",
      "POSITION_MISMATCH:OTHR4",
      "POSITION_MISMATCH:TEST3",
    ]);
    expect(local.cash).toBe("1000");
  });
  it("fails on incomplete, stale, duplicate and cross-account snapshots", () => {
    expect(
      reconcileAccount(snapshot(), snapshot({ complete: false }), options)
        .matches,
    ).toBe(false);
    expect(
      reconcileAccount(
        snapshot(),
        snapshot({ observedAt: "2026-09-10T14:00:00Z" }),
        options,
      ).discrepancies,
    ).toContain("BROKER_SNAPSHOT_TIME_INVALID");
    expect(
      reconcileAccount(snapshot(), snapshot({ accountId: "other" }), options)
        .discrepancies,
    ).toContain("ACCOUNT_MISMATCH");
    expect(
      reconcileAccount(
        snapshot(),
        snapshot({
          positions: [
            { symbol: "TEST3", quantity: 1 },
            { symbol: "TEST3", quantity: 1 },
          ],
        }),
        options,
      ).discrepancies,
    ).toContain("INVALID_RECONCILIATION_INPUT");
  });
  it("does not silently consider a corporate action split reconciled", () => {
    expect(
      reconcileAccount(
        snapshot(),
        snapshot({ positions: [{ symbol: "TEST3", quantity: 4 }] }),
        options,
      ).discrepancies,
    ).toContain("POSITION_MISMATCH:TEST3");
  });
});
