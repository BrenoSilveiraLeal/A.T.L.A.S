...
const values = {
  ...
  ATLAS_APP_URL: previous.ATLAS_APP_URL || "http://127.0.0.1:3000",
...
for (const value of Object.values(values)) if (/[\r\n]/.test(value)) throw new Error("Invalid multiline environment value.");
...
await writeFile(".supabase/atlas-owner-setup.html", `<!doctype html><html lang="pt-BR"><meta charset="utf-8"><title>Acesso inicial ATLAS</title><meta name="referrer" content="no-referrer"><h1>Defina seu acesso ao ATLAS</h1><p>Link temporário e pessoal. O servidor ATLAS deve estar em execução.</p><p><a href="${setupUrl}">Definir minha senha e depois cadastrar o autenticador</a></p></html>`, { mode: 0o600 });
